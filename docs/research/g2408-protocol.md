...
| 5.107 | **`energy_index`** (upstream `dreame-mower` `const.py:83`, comment "energy/discharge index property") | `{14, 15, 43, 56, 133, 158, 165, 176, 190, 196, 222, 237, 240, 250}` | Upstream stores the raw int and computes `energy_delta = new - old` on every update — i.e. treats it as a running cumulative quantity (`property/service5.py:215-237`). On g2408 our observed sequence isn't cleanly monotonic across a session (14 → 222 → 237 → 240 → 250 ... 56 at teardown), so it's NOT raw mWh consumed. More likely an enum-coded discharge-rate band, a rolling-window energy-state index, or something derived from cell voltage that oscillates with load. Dynamic, changes at session boundaries and mid-mow. Often fires alongside `s5p105` and `s5p106` updates in the same second (the same triplet pattern the SLAM-relocate event uses), but without an accompanying `s2p65` task-type string when it's just an energy-state push (vs a true relocate). 2026-04-24 mowing run added `240`; `56` is observed at session teardown; 2026-05-05 added `222` (09:40 mid-edge-mow) and `237` (10:46 mid-edge-mow). Surface as `sensor.energy_index` (diagnostic) once a g2408-specific value-to-meaning decode is verified. |
