...
| 1.2 | **`FIRMWARE_INSTALL_STATE`** | int | Firmware-update state machine, name from upstream `dreame-mower` `const.py:47`. Upstream value mapping: `2 = new_firmware_available`, `3 = installing_firmware_after_download`, `4 = firmware_download_failed`. **Our 2026-05-05 cloud `get_properties` returned `1`** (not in upstream's table) — best-guess "idle / up-to-date / no update activity" since the device wasn't updating. The property **almost never pushes via MQTT** — zero captures across 4 weeks of probe logs (2026-04-17 → 2026-05-05). Cloud `get_properties` does return a value so it is queryable on demand (cloud RPC unreliable on g2408, may need retry). Worth a one-shot poll at integration startup to surface "update available" if the cloud value is 2; not worth subscribing to for live updates since real updates fire so rarely. |
| 1.3 | **`FIRMWARE_DOWNLOAD_PROGRESS`** | int 0..100 | Firmware-update download percent, name from upstream `dreame-mower` `const.py:48`. `0` = no transfer in progress, `100` = downloaded. **Our 2026-05-05 cloud `get_properties` returned `0`**. Same MQTT-push profile as s1p2: zero captures in 4 weeks. Same one-shot-poll-at-startup recommendation. |
...
| 5.107 | **`energy_index`** (upstream `dreame-mower` `const.py:83`, comment "energy/discharge index property") | observed range 1–250 | Upstream stores the raw int and computes `energy_delta = new - old` on every update (`property/service5.py:215-237`). **Treat as push-on-change, not periodic.** A stable load (e.g. mowing flat ground at constant blade speed) may produce no push for tens of minutes; pushes mark transitions between discharge regimes (entering a slope, hitting a tuft, blade-load change). This is consistent with the observed sparse + irregular cadence (80 pushes across 4 weeks) and with `energy_delta` being the upstream's main consumer-facing signal. Stratifying the 80 captured pushes by mower state (`s2p1`) shows a clear load-weighted gradient — CHARGED (median 63) < CHARGING (100) < MOWING (median 133, n=53). Mowing's median is ~2× docked-charged's, matching the expected blade+drive+LiDAR vs idle-telemetry power ratio. Within-MOWING range (4–246) is wide enough to encode slope / turn-rate / blade-load. Pearson against battery-drop-rate over the preceding 10 min is +0.24 (n=40, 2026-04-17 → 2026-05-05) — direction-correct but noisy: battery percent is integer-quantized (1 % steps), s5p107's push cadence is event-driven not fixed-interval, and multi-session captures mix ambient confounders. **Best framing**: each push reports the *current* discharge index (mWh-over-interval class) at the moment a change crossed a firmware threshold; a steady reading just means "no change worth reporting since last push". Often fires alongside `s5p105` / `s5p106` in the same second (mirrors the SLAM-relocate triplet shape but without an accompanying `s2p65` task-type string when it's just an energy push). Surface as `sensor.energy_index` (diagnostic) once flat-vs-slope decode is confirmed; consumers should remember a stale value is still authoritative until the next push. **Future contributor opportunity**: the user's lawn is flat, so the slope-vs-flat distribution test can't be run from this device. Once the integration is public, a user with a sloped lawn can compare s5p107 distributions during the slope and flat segments of the same session — if median s5p107 rises with slope, the mWh-over-interval interpretation is confirmed. |
...
| [3] `& 0x80` | **Lift lockout / PIN required** (the app calls this *"Emergency stop is activated"*). Set when a lift OR a top-cover-open triggers the security lockout — the mower refuses to operate until a PIN is typed on-device. **Re-confirmed 2026-05-04 across a 5-test controlled series**: the bit clears ONLY on PIN entry; lid close, set-down, or any other physical-state restoration does NOT clear it. Smoking-gun was the dock-only test (lid open → lid close, NO PIN typed) where the bit stayed asserted after the lid closed. Then a follow-up test where the user opened lid → typed PIN → closed lid showed byte[3] cleared at PIN time (lid still open), confirming the trigger is the PIN, not the lid. |
...
| [10] `& 0x02` | **One-shot active-alert flag** (paired with the Dreame app's "Emergency stop activated" push notification + the mower's red LED + voice prompt). **Pinned down 2026-05-04** across a 5-test controlled series: sets ~1 s after byte[3] bit 7 sets (i.e. shortly after the safety event); **self-clears 30–90 s later regardless of state** — including while the lid is still open and PIN has not been entered. Variable timer (4 / 18 / 33 / 53 / 77 s observed) suggesting it's reset by sensor activity or an internal alert-window timer, not a fixed value. **NOT a "PIN-acceptance secondary latch"** as the earlier hypothesis claimed — the smoking-gun was the dock-only test where byte[10] cleared at 20:20:24 with the lid still open and no PIN ever typed. Independent of the byte[3] bit 7 lockout (which only clears on PIN). The base `0x80` bit (latched low-temp flag, see above) stays asserted independently; only bit 1 is the alert. Surfaced as `binary_sensor.safety_alert_active`. |
