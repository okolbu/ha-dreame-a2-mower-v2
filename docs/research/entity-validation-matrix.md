...
- **Outcome**: ✓ end-to-end live-confirmed 2026-05-09 — user physically read the LCD after a HA-driven flip and opened the lid to verify; LCD picker order captured. Cloud round-trip and value-preservation both ✓. Note: HA writes go to the cloud cache; if the device is in a state where it doesn't pull cache (e.g. anti-theft / locked / sleeping), the LCD may stay on its previous value until next wake — observed 2026-05-09 when text=20 was written but the LCD still showed English (mower was unattended).
- **Indexing**: 0-based on g2408's LCD. `text=0=Danish, 1=German, 2=English, 3=Spanish, 4=French, 5=Italian, 6=Dutch, 7=Norwegian, 8=Polish, 9=Finnish, 10=Swedish, 11=Simplified Chinese, 12=Traditional Chinese`. The two Chinese entries are best-guess from glyph descriptions ("box with vertical line through" / "stick-man") — confirm by setting one and checking the LCD.
- **Caveats**: out-of-range indices (e.g. text=20 we wrote earlier from the wrong app-picker mapping) get cached but the LCD doesn't apply them. `dreame_a2_mower.set_language` service writes any int 0..99 to bypass the static list. The Dreame app does NOT and is NOT EXPECTED TO reflect changes — its own *Languages* picker is the app's UI display language (separate setting; that's `APP_TEXT_LANGUAGE_NAMES` in `select.py` — kept as documentation, not used by any HA entity).
- **Recipe**: T4 — set a known language via HA, walk to the mower, open lid, verify the LCD displays the chosen language.
- **Verified**: ✓ end-to-end live 2026-05-09 / fw 4.3.6_0550 / int v1.0.3a5 — LCD list captured + cloud-cache wire format + device-apply path all confirmed.
...
### `button.dreame_a2_mower_request_wifi_map` — Refresh WiFi heatmap (cache-only on g2408)
- **Original intent (per ioBroker.dreame v0.3.7):** trigger a fresh device-side WiFi heatmap render via direct MIoT `s6.aiid=4`, wait 30s, fetch resulting OSS object.
- **Live test 2026-05-09:** `s6.aiid=4` returns 80001 — the siid=6 RPC tunnel is closed on g2408 firmware. The trigger path is broken.
- **BUT:** routed-action `g.OBJ d={type:'wifimap'}` returned 2 cached wifi map objects (auto-generated by the mower at unknown trigger times). The mower does generate wifi maps on its own schedule; we just can't request fresh ones from the integration.
- **Repurposed in v1.0.3a7:** the button now refreshes the integration's *cache* of the latest wifi map from OSS (`coordinator._refresh_wifi_map → cloud_client.fetch_wifi_map → camera.dreame_a2_mower_wifi_heatmap` updates). The button no longer dispatches the broken `s6.aiid=4` action.
- **Outcome**: ✓ end-to-end live-confirmed 2026-05-09 — pressing fetches the latest cached wifi map from OSS, decodes JSON, updates the camera. **Does NOT trigger a fresh device-side scan** — see TODO "Trigger fresh WiFi heatmap render" for that gap.
- **Recipe**: T6 — press, watch `camera.dreame_a2_mower_wifi_heatmap` populate.
- **Verified**: ✓ refresh-from-cache live 2026-05-09 / fw 4.3.6_0550 / int v1.0.3a7. Trigger-fresh path: ✗ closed, deferred to Phase 3 sniff.

### `camera.dreame_a2_mower_wifi_heatmap` — WiFi heatmap render (NEW v1.0.3a7)
- **Read**: cached wifi map from OSS (auto-generated by the mower on its own schedule). Format: `{data: list[int], width, height, resolution, startX, startY}`. RSSI integers in dBm; `1` = no data.
- **Render**: `wifi_map_render.render_wifi_map_png` → standalone heatmap PNG, red (-99 dBm) → yellow → green (-50 dBm), cells with no data left blank. Each cell is 32×32 px so a 16×18 wifi map → 512×576 PNG.
- **Refresh**: triggered by `button.request_wifi_map` (manual). No automatic poll yet — the user-driven refresh keeps the OSS API call rate low and matches the mower's irregular auto-scan schedule.
- **Future**: overlay on the live map renderer using the cloud-frame coordinates (`startX`/`startY`/`resolution` are in the same frame). See TODO "WiFi heatmap overlay on the live map".
- **Verified**: ✓ live 2026-05-09 — JSON decoded, format catalog confirmed (288 cells = 16×18, RSSI range -61 to -99, half cells empty), PNG render path tested.
