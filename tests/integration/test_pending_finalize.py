"""Pending-finalize wait task: completes on the dock (charging) signal or timeout.
...
Under the capture-until-docked lifecycle (Task 12) the only production
signal that ends the wait early is the physical-dock (charging) event;
the task_idle branch was removed so dock-return capture isn't cut off.
The wait method itself just relays whatever reason the signal sets, so the
early-resolution test below sets the reason directly.
"""
...
@pytest.mark.asyncio
async def test_wait_resolves_early_when_event_fires():
    """Resolves with the set reason when the event fires before timeout.

    The wait method relays whatever ``_pending_finalize_done_reason`` the
    signal set. In production the only reason that fires is 'charging'
    (see the next test); this case just pins the early-resolution plumbing.
    """
    coord = MagicMock()
...
async def test_wait_times_out_when_no_signal():
    """Returns 'timeout' when the dock (charging) signal does not fire in time."""
    coord = MagicMock()
    coord._pending_finalize_done = None
    coord._pending_finalize_done_reason = None
